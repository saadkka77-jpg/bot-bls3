...
def auction_embed(auction: dict[str, Any]) -> discord.Embed:
    color = COLOR_SECRET if auction["kind"] == "hidden" else ITEM_DEFINITIONS[auction["item_key"]]["color"]
    embed = base_embed(color)
    ...
    if auction["kind"] == "hidden":
        ...
        item_line = f"{item['icon']} العنصر: `{item['label']}`"
...
async def update_auction_message(auction: dict[str, Any]) -> None:
    ...
    try:
        ...
        await message.edit(embed=auction_embed(auction), view=AuctionActionView())
    except discord.NotFound:
        pass
    except discord.HTTPException:
...
async def post_auction(auction: dict[str, Any]) -> None:
    ...
    if not isinstance(channel, discord.TextChannel):
        return
    message = await channel.send(embed=auction_embed(auction), view=AuctionActionView())
...
async def process_company_income() -> None:
    ...
    for user in data_store["users"].values():
        ...
        if companies <= 0:
            user["lastCompanyTick"] = now
            continue
        last_tick = float(user.get("lastCompanyTick", now))
        cycles = int((now - last_tick) // COMPANY_TICK_SECONDS)
        if cycles <= 0:
            continue
        total_delta = 0
        for _ in range(cycles):
            for _company in range(companies):
                total_delta += resolve_company_change()
        user["money"] += total_delta
        user["lastCompanyTick"] = last_tick + (cycles * COMPANY_TICK_SECONDS)
        changed = True
    if changed:
        mark_dirty()
...
def resolve_steal_amount(victim_money: int) -> int:
    ...
    if victim_money <= 0:
        return 0
    if roll < 0.60:
        return max(1, victim_money // 4)
    if roll < 0.92:
        return max(1, min(victim_money, random.randint(500, max(500, victim_money // 3))))
    return min(victim_money, 40000)
...
class AuctionActionView(discord.ui.View):
    ...
    async def cancel_button(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        if interaction.user.id != auction.get("creator_id") and not is_admin:
            await interaction.response.send_message("فقط صاحب المزاد أو الإدارة يقدرون يلغونه.", ephemeral=True)
            return
        seller = get_user(auction["creator_id"])
    ...
    try:
        ...
        if cmd == "مزاد":
            await message.reply(
                ...
                view=LauncherView("player_auction"),
            )
            return
        if cmd == "قرض":
            if len(args) == 1:
        ...
        if cmd == "سداد":
            if len(args) == 1:
                ...
                await message.reply(
                    ...
                    view=LauncherView("repay", current_amount),
                )
                return
            amount = parse_amount(args[1])
            if user.get("loan"):
        ...
        if cmd == "بيع" and len(args) >= 2 and args[1] == "شركة":
            ...
            for _ in range(quantity):
                if random.random() < 0.03:
                    ...
                    total_sale += random.randint(450000, 650000)
            user["companies"] -= quantity
            credit_money(user, total_sale)
            save_user(user)
            await message.reply(embed=card_embed("تم بيع الشركة", f"{quantity} شركة مقابل {total_sale}", COLOR_WARNING, "🏢"))
            return
        if cmd == "شراء":
            if len(args) < 3:
